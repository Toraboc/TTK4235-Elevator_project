...
begin
    ...
    declare
        type CountArray is array (0..9) of Integer;
        expectedCounts: CountArray := (3, 3, 3, 2, 2, 2, 2, 2, 1, 0);
        observedCounts: CountArray := (others => 0);
        allOk: Boolean := True;
    begin
        if Integer(executionOrder.Length) /= 20 then
            allOk := False;
        end if;

        for idx in executionOrder.Iterate loop
            declare
                idVal: Integer := executionOrder(idx);
            begin
                if idVal in observedCounts'Range then
                    observedCounts(idVal) := observedCounts(idVal) + 1;
                else
                    allOk := False;
                end if;
            end;
        ...
        for i in observedCounts'Range loop
            if observedCounts(i) /= expectedCounts(i) then
                allOk := False;
            end if;
        end loop;
        ...
        if allOk then
            ...
            Put_Line("Tests failed: execution order does not match expected counts/length");
        end if;
    end;
